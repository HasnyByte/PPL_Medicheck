...
import { users as usersApi, bookings as bookingsApi, stats as statsApi, screenings as screeningsApi, type ApiUser, type Booking, type AdminStats, type Screening } from "@/lib/api";
import {
  Users,
  Activity,
  Calendar,
...
export function AdminDashboard({ user }: AdminDashboardProps) {
  ...
  const loadAll = useCallback(() => {
    ...
    Promise.all([
      ...
      const mappedUsers: SystemUser[] = apiUsers.map((u: ApiUser) => ({
        ...
        joinDate: new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(u.createdAt)),
        lastActive: "-",
      }));
      setUsers(mappedUsers);
      const mappedBookings: Appointment[] = apiBookings.map((b: Booking) => ({
        id: b.id,
        patient: b.user?.name || "Pasien",
        doctor: typeof b.doctor === "object" ? (b.doctor as {name?: string})?.name || "Dokter" : b.doctor || "Dokter",
        date: new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(b.date)),
        time: b.time,
      ...
      setApiStats(s);
      // Build activity logs from real booking data
      const logs: ActivityLog[] = apiBookings.slice(0, 20).map((b: Booking) => ({
        id: b.id,
        user: b.user?.name || "Pasien",
        action: `Booking dengan ${b.doctor?.name || "Dokter"} — ${b.disease} (${b.status})`,
  ...
  const totalPatients = apiStats?.patients ?? users.filter((u) => u.role === "patient").length;
  const totalDoctors = apiStats?.doctors ?? users.filter((u) => u.role === "doctor").length;
  const totalAppointments = apiStats?.totalBookings ?? appointments.length;
  const pendingAppointments = apiStats?.pendingBookings ?? appointments.filter((a) => a.status === "Menunggu").length;
  const confirmedAppointments = appointments.filter((a) => a.status === "Terkonfirmasi").length;
  ...
  return (
    <div style={{ minHeight: "calc(100vh - 64px)", background: "#f8fcfa" }}>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "40px 32px" }}>
        ...
        {activeTab === "logs" && <LogsTab logs={activityLogs.length > 0 ? activityLogs : MOCK_ACTIVITY_LOGS} />}
      </div>
    </div>
  );
}
...
function UsersTab({
  ...
  const saveEdit = () => {
    ...
    usersApi.update(editingUser.id, payload)
      .then((updated) => {
        setUsers((prev) => prev.map((u) => u.id === editingUser.id ? { ...u, name: updated.name, email: updated.email, phone: updated.phone || "-", role: updated.role as UserRole } : u));
        showToast(`Data ${updated.name} berhasil diperbarui`, "success");
      })
  ...
  const handleDelete = () => {
    ...
    usersApi.delete(deleteTarget.id)
      .then(() => {
  ...
  const [addPassword, setAddPassword] = useState("medicheck123");
  const handleAddUser = () => {
    if (!addForm.name || !addForm.email) {
      showToast("Nama dan email wajib diisi", "error");
      return;
    }
    usersApi.create({ name: addForm.name, email: addForm.email, password: addPassword, role: addForm.role, phone: addForm.phone || undefined })
      .then((created) => {
        const newUser: SystemUser = {
          ...
          status: "Aktif",
          joinDate: new Intl.DateTimeFormat("id-ID", { dateStyle: "medium" }).format(new Date(created.createdAt)),
      ...
      .catch((err) => showToast(err.message || "Gagal menambah pengguna", "error"));
